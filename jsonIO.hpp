...
void saveInOrder(string output, const Graph& G, const GridLayout& GL, int gridWidth, int gridHeight, int maxBends) {
	...
	node tmpNode = G.firstNode();
	int maxIndex = 0;
	while (tmpNode != nullptr) {
		if (tmpNode->index() > maxIndex) {
			maxIndex = tmpNode->index();
		}
		tmpNode = tmpNode->succ();
	}

	std::unordered_map<int, int> mapOldIdNewId;

	int m = 0;
	node n = G.firstNode();
	while (n != nullptr) {
		j2["nodes"][m]["id"] = m;
		mapOldIdNewId.insert(std::make_pair(n->index(), m));
		j2["nodes"][m]["x"] = (int)GL.x(n);
		j2["nodes"][m]["y"] = (int)GL.y(n);
		n = n->succ();
		m++;
	...
	while (e != nullptr) {
		//auto it1 = mapIndexId.find(e->source()->index());
		auto it1 = mapOldIdNewId.find(e->source()->index());
		j2["edges"][m]["source"] = (*it1).second;
		//auto it2 = mapIndexId.find(e->target()->index());
		auto it2 = mapOldIdNewId.find(e->target()->index());
		j2["edges"][m]["target"] = (*it2).second;
		IPolyline bends = GL.bends(e);
		if (bends.size() > 0) {
			int l = 0;
...
// ----- CREATION D'UN Graph A PARTIR D'UN FICHIER JSON -----
void readFromJsonMove(string input, int diffX, int diffY, Graph& G, GridLayout& GL, int& gridWidth, int& gridHeight, int& maxBends) {
	std::ifstream inp(input);
	json j;
	inp >> j;
	gridWidth = j["width"];
	...
	for (int i = 0; i < nodeNumber; i++) {
		nodeTab[i] = G.newNode();
		GL.x(nodeTab[i]) = j["nodes"][i]["x"] + diffX;
		GL.y(nodeTab[i]) = j["nodes"][i]["y"] + diffY;
		int id = j["nodes"][i]["id"];
		mapIndexId.insert(std::make_pair(nodeTab[i]->index(), id));
		mapIdNode.insert(std::make_pair(id, nodeTab[i]));
	}
	...
	for (int i = 0; i < edgeNumber; i++) {
		...
		if (j["edges"][i]["bends"] != nullptr) {
			...
			int k = 0;
			for (; k < bendsNumber; k++) {
				int bendX = j["edges"][i]["bends"][k]["x"] + diffX;
				int bendY = j["edges"][i]["bends"][k]["y"] + diffY;
				p.pushBack(IPoint(bendX, bendY));
				// On ajoute les bends dans la nodemap:
			}
			// ON AJOUTE LES BENDS SUPPLEMENTAIRE SUR LE NODE TARGET A LA FIN DES BENDS ORIGINAUX
...
// ----- CREATION D'UN Graph A PARTIR D'UN FICHIER JSON -----
void readFromJsonDoubleUpscale(string input, Graph& G, GridLayout& GL, int& gridWidth, int& gridHeight, int& maxBends) {
	std::ifstream inp(input);
	json j;
	inp >> j;
	gridWidth = j["width"];
	...
	for (int i = 0; i < nodeNumber; i++) {
		nodeTab[i] = G.newNode();
		GL.x(nodeTab[i]) = (int)(j["nodes"][i]["x"] * 100000.0);
		GL.y(nodeTab[i]) = (int)(j["nodes"][i]["y"] * 100000.0);
		int id = j["nodes"][i]["id"];
		mapIndexId.insert(std::make_pair(nodeTab[i]->index(), id));
		mapIdNode.insert(std::make_pair(id, nodeTab[i]));
	}
